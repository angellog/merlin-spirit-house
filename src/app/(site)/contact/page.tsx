...
import { getResolvedSiteSettings } from "@/sanity/lib/fetch";

export async function generateMetadata(): Promise<Metadata> {
  const settings = await getResolvedSiteSettings();
  const { clientTitle, clientName, clientLocation, clientDomain } = settings;
  return {
...
export default async function ContactPage() {
  const settings = await getResolvedSiteSettings();
  const { clientTitle, clientName, clientWhatsapp, clientEmail, clientLocation, clientDomain } = settings;
  const whatsappUrl = `https://wa.me/${cleanWhatsappNumber(clientWhatsapp)}?text=${encodeURIComponent(`Hello ${clientTitle} ${clientName}, I would like to get in touch.`)}`;
