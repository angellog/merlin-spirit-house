...
import { getPerson, getResolvedSiteSettings } from "@/sanity/lib/fetch";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const years = person?.yearsExperience ?? settings.clientYears;
  return {
    title: `About ${clientTitle} ${clientName} — ${years} Years of Spiritual Healing | Merlin Spirit House`,
    description: `Learn about ${clientTitle} ${clientName}, a traditional spiritual healer born into a lineage of Ugandan healers with over ${years} years of practice serving clients worldwide.`,
    alternates: { canonical: "/about/" },
    openGraph: {
      title: `About ${clientTitle} ${clientName} — ${years} Years of Spiritual Healing`,
      description: `Learn about ${clientTitle} ${clientName}, a traditional spiritual healer born into a lineage of Ugandan healers with over ${years} years of practice serving clients worldwide.`,
    },
  };
}
...
export default async function AboutPage() {
  ...
  const { clientTitle, clientName, clientWhatsapp, clientLocation, clientDomain } = settings;
  const years = person?.yearsExperience ?? settings.clientYears;
  const whatsappUrl = `https://wa.me/${cleanWhatsappNumber(clientWhatsapp)}?text=${encodeURIComponent(`Hello ${clientTitle} ${clientName}, I would like to book a private consultation.`)}`;
  ...
  const personSchema = {
    ...
    description: `Born into a lineage of traditional healers, ${clientTitle} ${clientName} has practiced ancestral spiritual healing for over ${years} years, serving international clients from Uganda, Kenya, UK, USA, Canada, South Africa, and Australia.`,
    ...
    sameAs: [
      `https://wa.me/${cleanWhatsappNumber(clientWhatsapp)}`,
    ],
  };
    ...
    <>
      ...
      <section className="min-h-[60vh] bg-stars flex flex-col items-center justify-center text-center px-6 py-24">
        ...
        <h1 className="max-w-4xl">
          About {clientTitle} {clientName} — {years} Years of Spiritual Healing
        </h1>
        <p className="mt-6 max-w-2xl text-lg leading-relaxed font-[family-name:var(--font-serif)] italic text-[var(--color-text-secondary)]">
          ...
          <div className="grid gap-16 lg:grid-cols-2 lg:items-start">
            <div className="flex justify-center">
              <div className="relative">
                <div className="h-[500px] w-full max-w-md rounded-2xl bg-[var(--color-bg-elevated)] border-2 border-[var(--color-gold-primary)]/30 overflow-hidden">
                  <div className="flex h-full items-center justify-center text-[var(--color-text-muted)]">
                    <span className="text-6xl">⚜</span>
                  </div>
                </div>
                <div className="absolute -bottom-3 -right-3 h-[500px] w-full max-w-md rounded-2xl border-2 border-[var(--color-gold-primary)]/20 -z-10" />
              </div>
            </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid gap-6 sm:grid-cols-3 pt-4">
                <div className="rounded-xl bg-[var(--color-bg-surface)] border border-[var(--color-gold-dim)]/30 p-6 text-center">
                  <p className="font-[family-name:var(--font-heading)] text-3xl font-bold text-[var(--color-gold-primary)]">{years}+</p>
                  <p className="mt-2 text-sm text-[var(--color-text-muted)]">Years of Practice</p>
                </div>
                <div className="rounded-xl bg-[var(--color-bg-surface)] border border-[var(--color-gold-dim)]/30 p-6 text-center">
                  <p className="font-[family-name:var(--font-heading)] text-3xl font-bold text-[var(--color-gold-primary)]">8+</p>
                  <p className="mt-2 text-sm text-[var(--color-text-muted)]">Countries Served</p>
                </div>
                <div className="rounded-xl bg-[var(--color-bg-surface)] border border-[var(--color-gold-dim)]/30 p-6 text-center">
                  <p className="font-[family-name:var(--font-heading)] text-3xl font-bold text-[var(--color-gold-primary)]">7</p>
                  <p className="mt-2 text-sm text-[var(--color-text-muted)]">Spiritual Services</p>
                </div>
              </div>

              <div className="mt-8 space-y-4">
                <h3>Countries Where I Have Helped Clients</h3>
                <p className="text-[var(--color-text-secondary)]">Uganda, Kenya, South Africa, Nigeria, United Kingdom, United States, Canada, Australia, Dubai, Saudi Arabia, Germany, and the Netherlands.</p>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <Link
                      key={s.href}
                      href={s.href}
                      className="rounded-full border border-[var(--color-gold-dim)]/40 bg-[var(--color-bg-surface)] px-4 py-1.5 text-xs text-[var(--color-gold-primary)] transition-colors hover:border-[var(--color-gold-primary)] hover:bg-[var(--color-gold-primary)]/10"
                    >
                      {s.label}
                    </Link>
                  ))}
      ...
      <section className="py-24">
        <div className="mx-auto max-w-5xl px-6 text-center">
          ...
          <div className="mt-12 grid gap-8 sm:grid-cols-2">
            <div className="rounded-2xl bg-[var(--color-bg-surface)] border border-[var(--color-gold-dim)]/20 p-8 text-left">
              <span className="text-2xl text-[var(--color-gold-primary)]">◈</span>
              <h3 className="mt-4">Inherited Spiritual Authority</h3>
              <p className="mt-3 text-[var(--color-text-secondary)]">Unlike those who learn from books alone, {clientName} carries the power of an unbroken ancestral lineage — a spiritual authority that cannot be taught, only inherited.</p>
            </div>
            <div className="rounded-2xl bg-[var(--color-bg-surface)] border border-[var(--color-gold-dim)]/20 p-8 text-left">
              <span className="text-2xl text-[var(--color-gold-primary)]">◈</span>
              <h3 className="mt-4">Distance Is No Barrier</h3>
              <p className="mt-3 text-[var(--color-text-secondary)]">Spiritual energy transcends physical distance. Clients in London, Dubai, Toronto, and Sydney receive the same powerful results as those who sit in the healing room in {clientLocation}.</p>
            </div>
            <div className="rounded-2xl bg-[var(--color-bg-surface)] border border-[var(--color-gold-dim)]/20 p-8 text-left">
              <span className="text-2xl text-[var(--color-gold-primary)]">◈</span>
              <h3 className="mt-4">Absolute Confidentiality</h3>
              <p className="mt-3 text-[var(--color-text-secondary)]">Every consultation, every ritual, every word shared is held in the strictest confidence. Your privacy is sacrosanct — this is not merely policy, it is spiritual law.</p>
            </div>
            <div className="rounded-2xl bg-[var(--color-bg-surface)] border border-[var(--color-gold-dim)]/20 p-8 text-left">
              <span className="text-2xl text-[var(--color-gold-primary)]">◈</span>
              <h3 className="mt-4">Results That Speak</h3>
              <p className="mt-3 text-[var(--color-text-secondary)]">The proof is in the lives transformed — relationships restored, curses broken, businesses revived, protection established. The work speaks for itself, and the testimonials confirm it.</p>
            </div>
          </div>
        </div>
      </section>
