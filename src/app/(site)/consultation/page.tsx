...
import { getConsultation, getResolvedSiteSettings } from "@/sanity/lib/fetch";
...
export default async function ConsultationPage() {
  const [settings, consultation] = await Promise.all([
    getResolvedSiteSettings(),
    getConsultation(),
  ]);

  const { clientTitle, clientName, clientWhatsapp, clientEmail, clientDomain } = settings;
  const whatsappUrl = `https://wa.me/${cleanWhatsappNumber(clientWhatsapp)}?text=${encodeURIComponent(`Hello ${clientTitle} ${clientName}, I would like to book a free private consultation.`)}`;

  const heroCopy = consultation?.heroCopy ?? `No obligation. No pressure. Just honest spiritual guidance from ${clientTitle} ${clientName}. Tell me what you are going through, and I will tell you what can be done.`;

  const whatHappensCopy = consultation?.whatHappensCopy ?? [
    {
      _type: "block",
      children: [{ _type: "span", text: `When you reach out, ${clientTitle} ${clientName} will personally review your situation — not an assistant, not an automated system, but the healer himself. You will describe what you are experiencing, whether it is a relationship crisis, financial hardship, a sense of spiritual attack, or something you cannot quite name. Everything you share is held in absolute confidentiality.` }],
    },
    {
      _type: "block",
      children: [{ _type: "span", text: `Based on what you describe, ${clientName} will perform a spiritual assessment — a reading of the energies, forces, and influences surrounding your situation. This is not guesswork or cold reading; it is a genuine spiritual evaluation rooted in decades of practice and ancestral authority. You will receive clear, honest feedback about what is happening and what can be done.` }],
    },
    {
      _type: "block",
      children: [{ _type: "span", text: `If the situation calls for spiritual work — a ritual, a spell, a cleansing, or protection — you will be told exactly what is recommended and why. There is no pressure to proceed. You are free to take the guidance and decide in your own time. The consultation is free because ${clientName} believes that everyone deserves honest spiritual guidance before making any commitment.` }],
    },
  ];

  const whatYouLearn = consultation?.whatYouLearn ?? [
  ...
  const privacyGuarantee = consultation?.privacyGuarantee ?? [
    {
      _type: "block",
      children: [{ _type: "span", text: "Every consultation is strictly confidential. Your name, your situation, and everything you share is protected by sacred oath. No information is ever shared with third parties, stored in insecure systems, or used for any purpose beyond your healing." }],
    },
  ];

  const guaranteeCopy = consultation?.guaranteeCopy ?? "If you are unsatisfied with the initial consultation, you owe nothing. There is no risk. The consultation is free, confidential, and carries no obligation. Reach out and see for yourself.";
    ...
    <>
      ...
      <section className="py-24">
        <div className="mx-auto max-w-4xl px-6">
          ...
          <div className="mt-12 text-lg leading-relaxed text-[var(--color-text-secondary)]">
            <PortableTextRenderer value={whatHappensCopy} />
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-3xl px-6">
          <div className="rounded-2xl border-2 border-[var(--color-gold-primary)]/40 bg-[var(--color-bg-surface)] p-8 text-center">
            ...
            <div className="mt-3 text-[var(--color-text-secondary)]">
              <PortableTextRenderer value={privacyGuarantee} />
            </div>
          </div>
        </div>
      </section>
